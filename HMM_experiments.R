...
data_all_ts<-features.full.svt.train.df[,1:9]
data_all_ts<-data_all_ts[,c(-2,-4,-9)] %>% as.matrix()
...
#observations deja sauvegarde dans data/full_features.RData
# observations<-c()
...
epmatrix<-table(features.full.svt.train.df$label,as.character(observations))
for (i in 1:11) {
...
nfold = 5
dataset <- create_folds_data(features.full.svt.train.df[,1:9], features.full.svt.test.df[,1:9], nfold = nfold)
precision <- numeric()
recall <- numeric()
F1_score <- numeric()
